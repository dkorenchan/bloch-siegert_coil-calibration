 function [w1_Hz,offsetsHz,Rsq,correctedAmpls,ProfFitPars] = ...
     BScoilProfileCal(targetB1,proffitflg,verbose) 

% Initially written 11/21/2024 by DK
% Updated 2/13/2025 by DK

% This function takes data from a Bloch-Siegert coil calibration (spin-echo
% experient with an off-resonance pulse applied during the second half of
% the echo), and it calculates the nutation frequency as a function of the
...
%                   amplitudes using coil profile fitting or not (default:
%                   false)
%   verbose     --  Logical indicating whether the fit at each offset to 
%                   obtain the Bloch-Siegert shift is plotted (true) or not
...
tableVals=Read_Tecmag_Header_Table(fname,{'o1_0:3','de0:2','rf0:3'});
...
if size(rawdata,2)<length(offsetsHz)
    offsetsHz=offsetsHz(1:size(rawdata,2));
    Ampls=Ampls(1:size(rawdata,2));
end
...
%     fitdels=delaysS(SNRtPass);
    ...
    thresh=0.2;
    initIdx=1:3;
%% I was going to figure out how to ID what values to average at the beginning for the slpJumps....    
%AH! We should be able to calculate what the slope jump would be if the
%value jumps by ~2pi!
%     initIdx=...

%     slpJumps=(abs(ptwiseSlp(1)-ptwiseSlp)>abs(ptwiseSlp(1)*thresh));
    slpJumps=(abs(mean(ptwiseSlp(initIdx))-ptwiseSlp)>abs(mean(ptwiseSlp(initIdx))...
        *(1+thresh)));
    % Fit the slope of the first points up to the first slope jump 
...
%             -(fitpts(jj+1:end)-fitpts(jj))+preIntrcpt)./2./pi);
%         add2pis=n2pis*2*pi;  
%         fitpts(jj+1:end)=fitpts(jj+1:end)+add2pis;
%     end

    for jj=1:5
        %Break out of for() loop if no more slope jumps
        if sum(slpJumps)<1
            break;
        end
        % Find first jump in slope
        firstJumpIdx=find(slpJumps,1);
        % Shift all points after the slope jump to match the initial 
        % pointwise slope
        n2pis=round((preSlp*(fitdels(firstJumpIdx+1:end)-fitdels(firstJumpIdx))...
            -(fitpts(firstJumpIdx+1:end)-fitpts(firstJumpIdx))+preIntrcpt)./2./pi);
        add2pis=n2pis*2*pi;  
        fitpts(firstJumpIdx+1:end)=fitpts(firstJumpIdx+1:end)+add2pis;
        % Then see if there are any remaining slope jumps by recalculating
        dS=diff(fitpts);
        ptwiseSlp=dS./dT;
        slpJumps=(abs(mean(ptwiseSlp(initIdx))-ptwiseSlp)>abs(mean(ptwiseSlp(initIdx))...
            *(1+thresh)));        
...
%         end
        ...
        if proffitflg %fit to coil profile
%             disp(['Correcting inputted off-resonance pulse amplitudes where '...
%                 'Bloch-Siegert is invalid using Lorentzian fitting...'])
            disp(['Correcting inputted off-resonance pulse amplitudes where '...
                'Bloch-Siegert is invalid by fitting to the expected resonant '...
                'circuit response curve...'])
